...
def _transform_images(is_ccrop=False):
    def transform_images(x_train):
        x_train = tf.image.resize(x_train, (112, 128))
...
if __name__ == '__main__':
    ...
    train_dataset = load_tfrecord_dataset(
        r'E:\Transformer\data\TFrecord\train-data-500-bin.tfrecord', batch_size, binary_img=True,
        is_ccrop=False)
    #dataset = train_dataset.map(lambda x, y: add_mask((x, y)))
    import matplotlib.pyplot as plt

    sample_dataset = dataset.take(5)  # 假設取前五個樣本

    # 將圖片和標籤拆分開來
    sample_images, sample_labels = zip(*sample_dataset)
