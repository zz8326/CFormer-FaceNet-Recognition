...
def _regularizer(weights_decay=1e-4):
...
def ArcHead(num_classes, margin1=0.5, margin2=0, margin3=0, logist_scale=64, name='ArcHead'):
    """Arc Head"""
    ...
    return arc_head
def ArcHead_v2(num_classes, s=64.0, m = 0.5, easy_margin=False, name='ArcHead'): #out_features, s=64.0, m=0.50, easy_margin=False
    """Arc Head"""
    def arc_head(x_in, y_in):
        x = inputs1 = Input(x_in.shape[1:])
        y = Input(y_in.shape[1:])
        x = ArcFace(out_features=num_classes,
                                    s = s,
                                    m = m,
...
def NormHead(num_classes, w_decay=5e-4, name='NormHead'):
    ...
    def norm_head(x_in):
        x = inputs = Input(x_in.shape[1:])
        x = Dense(num_classes, kernel_regularizer=_regularizer(w_decay), activation = 'softmax')(x)
        return Model(inputs, x, name=name)(x_in)
    return norm_head
